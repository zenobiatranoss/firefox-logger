...
const config = {
    mode: "detailed" as "simple" | "detailed"
}
...
function send(event: Record<string, unknown>) {
    ...
    try {
        console.error("[SEND ATTEMPT]"); console.error("[SEND DATA]", JSON.stringify(event.data))
        port.postMessage({
            type: "event",
            event
        })
...
function prepareData(data: Record<string, unknown>) {
    if (config.mode !== "detailed") return undefined

    const requestBody = data.requestBody as {
        formData?: Record<string, string[]>
        raw?: Array<{ bytes?: unknown }>
        lenientFormData?: unknown
    } | undefined

    if (!requestBody) return data
    ...
    if (requestBody.raw) {
        result.requestBody = {
            raw: requestBody.raw.map(part => {
                ...
                if (bytes && typeof (bytes as any).byteLength === "number") {
...
function prepareHeaders(headers: browser.webRequest.HttpHeaders | undefined) {
...
browser.webRequest.onBeforeRequest.addListener(
    details => {
        const body = (details as any).requestBody
        ...
        const preparedBody = (prepared as any)?.requestBody?.raw?.[0]?.bytes
        ...
        send(createEvent("request", {
            ...
            initiator: (details as any).initiator,
